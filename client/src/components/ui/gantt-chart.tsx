 ...
 * ===============================================================================
 *
 * This Gantt chart component provides a flexible timeline view for project management.
 ...
 *     startAt: new Date(2024, 0, 15), // Jan 15, 2024
 *     endAt: new Date(2024, 1, 5), // Feb 5, 2024
 *     status: statuses[3], // Done
 *     lane: "planning", // Features with same lane share a row
 *   },
 *   // ... more features
 * ];
 *
 ...
 * const [range, setRange] = useState<Range>("monthly"); // "daily" | "monthly" | "quarterly"
 *
 * // Group features by lane for organized display
 * const groupFeaturesByLane = (features: GanttFeature[]) => {
 ...
 * };
 *
 * const groupedFeatures = groupFeaturesByLane(features);
 *
 * STEP 3: Create event handlers
 *
 * const handleAddFeature = (date: Date) => {
 *   // Add new feature when clicking on timeline
 * };
 *
 * const handleMoveFeature = (id: string, startAt: Date, endAt: Date | null) => {
 *   // Update feature position when dragging
 * };
 *
 * STEP 4: Render the component structure
 *
 ...
 *     <GanttSidebar>
 *       <GanttSidebarGroup name="Planning">
 *         <GanttSidebarItem feature={myFeature} onSelectItem={handleSelect} />
 *       </GanttSidebarGroup>
 *     </GanttSidebar>
 *
 *     // Main timeline area
 ...
 *           <GanttFeatureRow features={myFeatures} onMove={handleMoveFeature}>
 *             {(feature) => (
 ...
 *
 *       <GanttToday />
 *       <GanttMarker id="m1" date={new Date()} label="Milestone" />
 *
 *     </GanttTimeline>
 *   </GanttProvider>
 * </div>
 ...
 * KEY CONCEPTS:
 *
 * 1. **GanttProvider**: Wraps everything, controls zoom (50-200%) and range ("daily", "monthly", "quarterly")
 *
 * 2. **Data Structure**:
 *    - GanttFeature: { id, name, startAt, endAt, status, lane }
 *    - GanttStatus: { id, name, color }
 *    - GanttMarkerProps: { id, date, label }
 *
 * 3. **Layout**: Two-column grid with sidebar (feature list) and timeline (visual bars)
 *
 * 4. **Lanes**: Features with same `lane` property are grouped in the same row, allowing for parallel tasks
 *
 * 5. **Interactive Features**:
 *    - Click timeline to add new features (if onAddItem provided)
 *    - Drag feature bars to move/resize (if onMove provided)
 *    - Right-click markers to remove (if onRemove provided)
 *    - Click sidebar items to select/highlight features
 *
 * 6. **Overlapping Features**: Automatically handled with sub-rows when features in same lane overlap
 *
 * 7. **Responsive**: Timeline extends infinitely in both directions with lazy loading
 *
 * The component handles complex timeline calculations internally, so you just need to provide your data and event handlers.
 */
